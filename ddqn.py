...
# Always remember to change the preprocessing() method when changing the environment
...
def train(doubleDQN = False, duelingDQN = False, gamma = 0.99, size = 1e6, log = True, render = False):
    ...
    replay_buffer = ReplayBuffer(size, input_frame_len)
    ...
    while iteration<total_iterations:
        ...
        if iteration<learning_starts:
            ...
            if sample > threshold:
                ...
                observations = replay_buffer.get_recent_observation()   # (4,84,84) numpy ndarray 
                # print(observations.shape)

                obs = torch.from_numpy(observations).unsqueeze(0).type(dtype=torch.float).cuda()
        ...
        replay_buffer.store_action_reward_done(last_stored_frame_idx, action, reward, done_store)
        ...
        if(iteration>learning_starts and iteration%learning_freq == 0 and replay_buffer.can_sample(batchsize)):

            obs_batch, act_batch, rew_batch, obs_next_batch, done_mask = replay_buffer.sample(batchsize)
            ...
            # print(q_values)
            # Compute Q(s, a) - the model computes Q(s_t), then select the columns of actions taken.
            q_value = q_values.gather(1, act_batch.unsqueeze(1)).squeeze(1)
            # print(q_value)

            # Compute Qpi(s,a) = rt + gamma * Qpi(st+1,argmaxQ(st+1;theta t);theta t)
            ...
            expected_q_value = rew_batch + gamma * next_q_value * (1 - done_mask)   # if current state is end, then there is no next Q value
...
# train(duelingDQN=True,render=False)

train(duelingDQN=True,render=False,log=True)

# test statement
# test(path = "./models/dueling_1400000.model",num_episodes = 1000,env=env,actionDim=actionDim,device=device,duelingDQN=True)
